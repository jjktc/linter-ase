...
module.exports = {
  ...
  checkSpacing: function(results, spacer) {
    ...
    this.editor.scan(new RegExp(spacerRegex, 'g'), iterator => {
      ...
      if (this.hasScope(scope, spacerScope) && this.hasTypicalScope(scope)) {
        // Found item, check for proper spacing now
        var properSpacing = true;

        if (textBefore != " " && textBefore != "\t" && !this.hasScope(scopeBefore, this.aseSpacerGoodBreak)) {
          // Found bad spacing in front
          properSpacing = false;
        }

        if (properSpacing && textAfter != " " && textAfter != "\t" && !this.hasScope(scopeAfter, this.aseSpacerGoodBreak)) {
        ...
        if (iterator.matchText == startSymbolStripped) {
          ...
          if (matchStack.length > 0) {
            ...
            for (var i = matchStack.length - 1; i >= 0; i--) {
              if (matchStack[i].characterIndex == iterator.range.start.column) {
